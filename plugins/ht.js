let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Esperate cabrón\nAlguien lo uso sntes que vos\nde este vchat... espera a que termine'
  else conn.hartatahta[m.chat] = true
  m.reply('_Haciendolo..._\n*Espere un momento*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'SusiBOT.png', m)
  } finally {
    delete conn.hartatahta[m.chat]
  }
}
handler.help = ['tahta ᴱˢᶜʳᶦᵇᵉ ᵃˡᵍᵒ']
handler.tags = ['tools']
